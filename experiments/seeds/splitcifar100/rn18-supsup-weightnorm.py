...
def main():
    ...
    config = "experiments/seeds/splitcifar100/configs/rn18-supsup-weightnorm.yaml"
    log_dir = "/scratch/{user}/runs/{logdir_prefix}/SupsupSeedWeightnorm/rn18-supsup".format(user=os.environ.get("USER"), logdir_prefix=args.logdir_prefix)
    experiments = []
    ...
    for seed in seeds:
        kwargs = {
            "config": config,
            "name": f"id=supsup-weightnorm~seed={seed}",
            "seed": seed,
            "log-dir": log_dir,
