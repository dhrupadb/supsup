...
def main():
    ...
    config = "experiments/seeds/splitcifar100/configs/rn18-supsup-hybrid-signed-weightnorm.yaml"
    log_dir = "/scratch/{user}/runs/{logdir_prefix}/SupsupSeedHybridSignedWeightnorm/rn18-supsup".format(user=os.environ.get("USER"), logdir_prefix=args.logdir_prefix)
    experiments = []
    ...
    for sparsity, seed in product(sparsities, seeds):
        kwargs = {
            "config": config,
            "name": f"id=supsup-hybrid-signed-weightnorm~seed={seed}~sparsity={sparsity}",
            "sparsity": sparsity,
            "seed": seed,
            "log-dir": log_dir,
